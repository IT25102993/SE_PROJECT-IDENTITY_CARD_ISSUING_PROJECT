...
import {
  ...
  AlertCircle, ArrowLeft, IdCard, Briefcase,
...
export const RegisterPage = () => {
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      ...
      addToast(data.message || `OTP sent to ${email}`, 'success');
      if (data.devOtp) {
        setOtp(data.devOtp);
      }
      setStep(STEP_OTP);
  ...
  // Step 2b → verify OTP then register
      ...
      <div className="glass-card animate-fade-in" style={{
        ...
        {step === STEP_FORM && (
          <form onSubmit={handleSendOtp}>
            ...
            {/* Email */}
            <div className="form-group">
              <label className="form-label" htmlFor="reg-email">Official Email Address</label>
              <div style={{ position: 'relative' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
                  {pwdReqs.map(req => (
                    <div key={req.label} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.78rem' }}>
                      <CheckCircle2 size={13} style={{ color: req.met ? 'var(--accent-emerald)' : 'var(--text-muted)', flexShrink: 0, transition: 'color 0.2s' }} />
            ...
            <button type="submit" className="btn btn-emerald" disabled={sendingOtp}
              ...
              {sendingOtp ? <span>Sending OTP...</span> : <><Send size={18} /> Send Verification Code</>}
            </button>
          </form>
        )}
        ...
        {step === STEP_OTP && (
          <form onSubmit={handleVerifyAndRegister}>
            ...
            <div style={{
              ...
              <span>A 6-digit code was sent to <strong style={{ color: 'var(--text-primary)' }}>{formData.email}</strong>. Check your inbox (and spam folder).</span>
            </div>
            ...
            <button type="button" onClick={() => { setStep(STEP_FORM); setOtp(''); setErrorMessage(''); }}
              ...
              <ArrowLeft size={15} /> Back to Edit Form
            </button>
          </form>
        )}
