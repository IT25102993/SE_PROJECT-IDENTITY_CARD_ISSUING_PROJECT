...
export const TrackingPage = () => {
  ...
  const performSearch = (term) => {
    ...
    const found = applications.find(
      a => (a.id && a.id.toLowerCase() === term.toLowerCase()) ||
  ...
  const STAGES = [
    ...
    { key: 'Verification', label: '2. Verification', icon: UserCheck },
    { key: 'Approved', label: '3. Approved', icon: CheckCircle2 },
    { key: 'Printed', label: '4. Card Printed', icon: Printer },
      ...
      <div className="container">
        <div style={{ textAlign: 'center', maxWidth: '650px', margin: '0 auto 2.5rem auto' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.95rem' }}>
            Check processing stage, verification updates, and delivery details of your NIC from the central database.
          </p>
        </div>
            ...
            <div className="glass-card animate-fade-in" style={{ padding: '2rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', fontSize: '0.9rem', marginBottom: '2rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.4rem' }}>
                  ...
                  <span style={{ fontWeight: 600 }}>{selectedApp.fullNameEn || `${selectedApp.first_name || ''} ${selectedApp.last_name || ''}`}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.4rem' }}>
                  ...
                  <span style={{ fontWeight: 700, color: '#fbbf24', fontFamily: 'var(--font-mono)' }}>
                    {selectedApp.nicNumber || selectedApp.national_id_number || 'Pending Assignment'}
                  </span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.4rem' }}>
                  <span style={{ color: 'var(--text-secondary)' }}>District:</span>
                  <span style={{ fontWeight: 600 }}>{selectedApp.district || 'Colombo'}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.4rem' }}>
                  <span style={{ color: 'var(--text-secondary)' }}>Submitted Date:</span>
                  <span style={{ fontWeight: 600 }}>{selectedApp.submittedDate || selectedApp.submitted_at || 'Recent'}</span>
                </div>
              </div>
            </div>
